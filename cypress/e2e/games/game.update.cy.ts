...
describe("Game update tests", () => {
  it("should be visible", () => {
    ...
    cy.loginAsAdmin()
    cy.get('[cy-data=game-list-item-edit-button]').eq(0).click()
  ...
  it("should show error if submit while blank", () => {
    ...
    cy.loginAsAdmin()
    cy.get('[cy-data=game-list-item-edit-button]').eq(0).click()
    cy.get('[cy-data=game-name]').clear()
    cy.get('[cy-data=game-boxart]').clear()
  ...
  it("should send correct request if submit is pressed", () => {
    ...
    cy.intercept('PUT', backUrl + '/games/1', (req) => {
